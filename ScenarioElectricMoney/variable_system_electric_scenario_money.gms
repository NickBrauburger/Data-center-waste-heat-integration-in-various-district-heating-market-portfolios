...
Table Money(T,D)     2 dimensional table
...
Cost_heat_production_DC(D,T) 'need to write something'
...
Total_cost_of_DC(D)..           C_T_D(D) =e= sum(T, OM_variable_DC(D)*x_d(D,T) - (Money(T,D))*x_d(D,T) + e(D,T)*eday(T) + OM_variable_infrastruture_DC(D)*x_d(D,T) + e(D,T)*0.536912752) + OM_fixed_DC(D)*(sum(T,Waste_heat(T))/8760)*(sum(T,COP_HP_DC(T,D))/8760)*z_g(D) + OM_nominal_DC(D)*(sum(T,Waste_heat(T))/8760)*(sum(T,COP_HP_DC(T,D))/8760)*z_g(D) + OM_nominal_infrastruture_DC(D)*data(D);
