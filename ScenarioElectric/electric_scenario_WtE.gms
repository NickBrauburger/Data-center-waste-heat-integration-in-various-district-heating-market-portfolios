...
*py.fx(G,T)$(not CHP_G_WtE_try(G) and not HOP_G_WtE_try(G)) = 0;
...
*Zero_HOP(G,T)..            py(G,T) - sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G))$HOP_G_WtE_try(G)) =e= Market_setter_G;
*Zero_CHP(G,T)..            px(G,T) - sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G) - (e_g(G,T)/c_v(G)))$CHP_G_WtE_try(G)) =e= Market_setter_G;
*cost of operation
*eday(T)

Cost_of_system..            Z =e= sum((D), C_T_D(D)) + sum((G), C_T_G(G)) - sum(T, sum(G, e_g(G,T))*(eday(T))) + sum(T,sum(G, py(G,T)))*0.01 + sum(T,sum(G, px(G,T)))*0.01;
...
Total_cost_of_DC(D)..           C_T_D(D) =e= sum(T, OM_variable_DC(D)*x_d(D,T) + e(D,T)*eday(T) + OM_variable_infrastruture_DC(D)*x_d(D,T) + e(D,T)*0.536912752) + OM_fixed_DC(D)*(sum(T,Waste_heat(T))/8760)*size(D) + OM_nominal_DC(D)*(sum(T,Waste_heat(T))/8760)*size(D) + OM_nominal_infrastruture_DC(D)*data(D);


Investment_cost(G)..        C_inv_g(G) =e= OM_fixed(G)*g_c(G) + OM_nominal(G)*g_c(G);
Investment_cost_DC(D)..        C_inv_DC(D) =e= OM_fixed_DC(D)*z_g(D)*size(D) + OM_nominal_DC(D)*z_g(D)*size(D) + OM_nominal_infrastruture_DC(D)*data(D); 
...
Heat_production_equation(G,T).. x_g(G,T) =e= sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G))$HOP_G_no_WtE(G)) + sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G) - py(G,T))$HOP_G_WtE_try(G)) + sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G) - (e_g(G,T)/c_v(G)))$CHP_G_no_WtE(G)) + sum(F, (x_f(G,F,T)*fuel_energy(F)*Thermal_Efficiency(G) - (e_g(G,T)/c_v(G)) - px(G,T))$CHP_G_WtE_try(G)) + (x_g(G,T))$Heat_pumps_G(G) + x_g(G,T)$Electric_HOP(G);
