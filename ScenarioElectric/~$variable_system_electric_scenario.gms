...
/
$ondelim
$include    capacities_municipal.csv
$offdelim
/
...
e_IH(G,T)           'electricty taken from grid to supply the industrial heat pump'
...
*cost of operation
*eday(T)
Cost_of_system..            Z =e= sum((D), C_T_D(D)) + sum((G), C_T_G(G)) - sum(T, sum(G, e_g(G,T))*(eday(T)));


*the variable cost of the CHP's needs to be in terms of heat since the extraction plants can operate at 0 electricty production, at least check it out

Cost_heat_production(G,T).. C_g(G,T) =e= (sum(F, x_f(G,F,T)*fuel_price(F)*fuel_energy(F)) + OM_variable(G)*x_g(G,T) + sum(F, x_f(G,F,T)*fuel_energy(F)*Fuel_taxes(F) + x_f(G,F,T)*fuel_energy(F)*CO2_taxes(F)))$(HOP_G(G)) + (OM_variable(G)*e_g(G,T) + sum(F, OM_fuel_usage(G)*x_f(G,F,T)*fuel_energy(F)) + sum(F, x_f(G,F,T)*fuel_energy(F)*Fuel_taxes(F) + x_f(G,F,T)*fuel_energy(F)*CO2_taxes(F)))$(CHP_G(G)) +  (cost_HP(G,T))$(Heat_pumps_G(G)) + cost_EL(G,T)$Electric_HOP(G);
Cost_heat_production_DC(D,T).. C_d(D,T) =e= cost_HP_DC(D,T);
Total_cost_of_G(G)..            C_T_G(G) =e= sum(T, (sum(F, x_f(G,F,T)*fuel_price(F)*fuel_energy(F)) + OM_variable(G)*x_g(G,T) + sum(F, x_f(G,F,T)*fuel_energy(F)*Fuel_taxes(F) + x_f(G,F,T)*fuel_energy(F)*CO2_taxes(F)))$(HOP_G(G)) + (OM_variable(G)*e_g(G,T) + sum(F, OM_fuel_usage(G)*x_f(G,F,T)*fuel_energy(F)) + sum(F, x_f(G,F,T)*fuel_energy(F)*Fuel_taxes(F) + x_f(G,F,T)*fuel_energy(F)*CO2_taxes(F)))$(CHP_G(G)) + (cost_HP(G,T))$(Heat_pumps_G(G)) + cost_EL(G,T)$Electric_HOP(G)) + OM_fixed(G)*Capacities(G) + OM_nominal(G)*Capacities(G) + OM_nominal_fuel(G)*Capacities(G); 


Total_cost_of_DC(D)..           C_T_D(D) =e= sum(T, OM_variable_DC(D)*x_d(D,T) + e(D,T)*eday(T) + OM_variable_infrastruture_DC(D)*x_d(D,T) + e(D,T)*0.536912752) + OM_fixed_DC(D)*(sum(T, Waste_heat(T)*z_g(D))/8760) + OM_nominal_DC(D)*(sum(T, Waste_heat(T)*z_g(D))/8760) + OM_nominal_infrastruture_DC(D)*data(D);


Investment_cost(G)..        C_inv_g(G) =e= OM_fixed(G)*Capacities(G) + OM_nominal(G)*Capacities(G) + OM_nominal_fuel(G)*Capacities(G);
Investment_cost_DC(D)..        C_inv_DC(D) =e= OM_fixed_DC(D)*(sum(T, Waste_heat(T)*z_g(D))/8760) + OM_nominal_DC(D)*(sum(T, Waste_heat(T)*z_g(D))/8760) + OM_nominal_infrastruture_DC(D)*data(D); 
*+OM_nominal_infrastruture_DC(D)+OM_variable_infrastruture_DC(D)*x_d(D,T) + OM_fixed_DC(D)*z_g(D) + OM_nominal_DC(D)*z_g(D) ++ sum(D, + OM_fixed_DC(D)*z_g(D))
...
*upper bound of energy production in generation technologies
Upper_bound_not_CHP(G,T)..      (x_g(G,T))$(not CHP_G(G)) =l= (Capacities(G))$(not CHP_G(G));
Upper_bound_electric_CHP(G,T)..      (e_g(G,T))$(CHP_G(G)) =l= (Capacities(G))$(CHP_G(G));
Upper_bound_CHP(G,T)..      (x_g(G,T))$(CHP_G(G)) =l= (Capacities(G)/c_v(G))$(CHP_G(G));


Lower_bound_extraction_CHP(G,T)..  (e_g(G,T))$CHP_extraction_G(G) =g= ((c_b(G)*x_g(G,T)))$CHP_extraction_G(G);
Upper_bound_extraction_CHP(G,T)..  (e_g(G,T))$CHP_extraction_G(G) =l= (Capacities(G) - (c_v(G)*x_g(G,T)))$CHP_extraction_G(G);
Production_equation_back_pressure_CHP(G,T).. (e_g(G,T))$CHP_back_pressure_G(G) =e= ((c_b(G)*x_g(G,T)))$CHP_back_pressure_G(G);


*Heating_demand(G,K,T).. sum(G, y_g(G,K,T)) - T_discharge(T) + T_charge(T) =e= Demands(K,T);
Heating_demand(K,T).. sum(G, y_g(G,K,T)*(1-Loss_Coefficient(G,K))) + sum(D, y_d(D,K,T)*(1-Losses_DC(D,K))) =e= Demands(T,K);

*production in generation tech, thermal efficieny of BIO CHP & COAL plant & gas turbines is very questinable ask question about it!!!
...
*latent storage tank(needs lots of work still)!!!
*Tank_charge(T)..       T_charge(T) =l= T_charge_rate*Capacities(G13);       
*Tank_discharge(T)..    T_discharge(T) =l= T_discharge_rate*Capacities(G13);       
*SOC_tank(T)..          SOC_T(T) =e= SOC_inital$(ord(t)=1) + SOC_T(T-1)$(ord(t)>1) - n_t*T_discharge(T) + T_charge(T);    
*upper_bound_SOC(T)..   SOC_T(T) =l= Capacities(G13)

Force(D).. z_g(D) =l= 75*data(D);
...
*industrial air source heat pump, no limit but capacity since it has an 'unlimeted' source
Heat_production_HP_industrial(G,T).. x_g(G,T)$Heat_pumps_G(G) =e= (COP_HP_industrial(T)*e_IH(G,T))$Heat_pumps_G(G);
*combined HP cost of heat production
...
*Waste_limit(G,F,T).. x_f(G,F,T)*fuel_energy(F) =l= Capacities(G);
